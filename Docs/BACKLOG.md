...
- [M0] Every push hashes every local payload (cached within one run, not across runs). Task 20 measured it: 0.45 s for a no-op sync at 2,000 tiny pages, linear in total payload bytes. Persist the hash next to the model (`Page.drawingHash` set by `DrawingStore.save`, likewise for imports) before libraries get large.
- [M0, before M1] **Compress the `.strokes` record.** Task 20 measured 1,440 KB for a 500-stroke page — ~6× PencilKit's encoding — and it's what syncs per page per edit. Set a header flag bit, compress the body (LZFSE or zlib via `Compression`), keep reading uncompressed v1. Do it before any real library is synced so there's nothing to migrate.
...
- [M0] **Invariant 4 has a hole without a sync folder.** `Documents/` holds ink but no notebook/page structure, so a corrupt SwiftData store on a device that never chose a folder is unrecoverable. Fix: keep a local mirror of the folder layout (`Documents/Library/index.json`, `notebooks/`, `tombstones.json`) written by the same debounced push, and let `LibraryRebuild` read from it when no sync folder is chosen. Cheap given the engine - a second `SyncEnvironment` whose folder is local. Worth doing before M1 ships to anyone.
